...
class HospitalAdkApp(AdkApp):
    def set_up(self):
        """This runs on the cloud server during startup."""
        # Optional: call the base setup if needed
        super().set_up() 
        
        from google.adk.tools.mcp_tool.mcp_toolset import MCPToolset, SseConnectionParams
        
        print("Initializing MCP tools in the cloud...")
        ...
        self.agent.tools = hospital_toolset.tools
# -----------------------------------------------------------------------------
...
agent_app = HospitalAdkApp(agent=root_agent)
